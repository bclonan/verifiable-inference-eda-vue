import type { ContextEnvelope, DTO, StackSpec, ViewSpec } from "../core/types.js";
...
function viewKey(viewSpecId: string, sessionId: string) {
...
export async function buildView(params: {
...
}): Promise<{ view: DTO; receiptId?: string; receipt?: any }> {
    const viewSpec: ViewSpec | undefined = VIEW_SPECS[params.viewSpecId];
    if (!viewSpec) throw new Error(`Unknown ViewSpec: ${params.viewSpecId}`);
    ...
    const stack: StackSpec = STACK_SPECS["semantic_extract@1"];
