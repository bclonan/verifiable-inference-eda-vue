...
export async function verifyReceipt(store, receiptId) {
    const receipt = await store.getReceipt(receiptId);
    if (!receipt)
        return { pass: false, reasons: ["Receipt not found"] };
    const reasons = [];
    ...
    const okReceiptSig = verifyPayloadHex(receipt.runtimeAttestation.signedBy, receiptPayload, receipt.receiptSignatureHex);
    if (!okReceiptSig)
        reasons.push("Receipt signature invalid");
    // Verify evidence refs exist (best-effort in v0.1)
    for (const ref of receipt.evidenceRefs) {
        const m = ref.match(/^cas:\/\/([a-f0-9]+)\//);
        if (!m) {
            reasons.push(`Bad evidence ref: ${ref}`);
            continue;
        }
        const h = m[1];
        const blob = await store.getMaterializedView(`evidence:${h}`);
        if (!blob)
            reasons.push(`Missing evidence blob for ${ref}`);
    }
